...
OUTPUT_IMAGE_PATH = "/home/ai/said/svm_merged/checkpoint/grid_svm_feature_importance.png"
OUTPUT_CONF_MATRIX_PATH = "/home/ai/said/svm_merged/checkpoint/grid_svm_confusion_matrix.png"
...
# drop_cols = ['FileName', 'SubjectID', 'Class', 'Split', 'Task', 'Hesitation', 'SEX', 'SMO', 'DRH','Num_PS', 'P_less_1', 'P_Sum','Var_PS', 'Avg_PS', 'Len_S','Rat_S','Num_S']
...
# 최종적으로 사용할 feature: MI filtering 제거 → 상관관계 제거 후 남은 모든 feature 사용
...
# GridSearchCV를 통한 최적의 SVM 하이퍼파라미터 탐색
# SVC는 kernel='rbf', class_weight='balanced', random_state=42를 고정하고, C와 gamma를 튜닝
param_grid = {
    'C': [0.1, 0.5, 0.7, 1, 3, 5, 7, 10,13,15, 100],
    'gamma': [0.001,0.005, 0.01,0.05, 0.1,0.5,0.7,1]
}
svm_estimator = SVC(kernel='rbf', class_weight='balanced', random_state=42)
grid_search = GridSearchCV(svm_estimator, param_grid, cv=5, scoring='accuracy', n_jobs=-1)
grid_search.fit(X_train_scaled, y_train)

print("GridSearch 최적 파라미터:", grid_search.best_params_)
print("GridSearch 최고 CV 스코어:", grid_search.best_score_)
...
perm_result = permutation_importance(best_svm, X_test_scaled, y_test, scoring='accuracy',
