...
def main():
    ...
    # MFCC 추출을 위한 torchaudio.transforms.MFCC 설정
    # n_mfcc=13 (0차 포함; 최종적으로 1~12만 사용)
    ...
    for wav_path in wav_files:
        ...
        # VAD를 이용해 음성 구간 추출 (샘플 단위로 start, end 반환)
        with torch.no_grad():
        ...
        mfcc_use = mfcc_all[0, 1:, :]  # [12, n_frames] (0차 제외)

        n_frames = mfcc_use.shape[1]
        # silent_frame_mask: True if frame_time is in a silence segment, else False
        silent_frame_mask = torch.zeros(n_frames, dtype=torch.bool)
        for i in range(n_frames):
            frame_time = i * hop_size / sr  # 시간 (초)
            # silent if frame_time falls within any silence segment
            for (silence_start, silence_end) in silence_segments:
                if silence_start <= frame_time < silence_end:
                    silent_frame_mask[i] = True
                    break

        # silent frame에 해당하는 MFCC 값을 0으로 설정
        if silent_frame_mask.any():
            mfcc_use[:, silent_frame_mask] = 0.0

        # Pearson 상관계수 기반 self-similarity 행렬 계산
        ssm = compute_correlation_matrix(mfcc_use)  # [n_frames, n_frames]

        # min-max 정규화 후 이미지 저장
        ...
        print(f"[{filename}] -> {len(silence_segments)} silence segments, {silent_frame_mask.sum().item()} silent frames, saved: {out_filename}")
