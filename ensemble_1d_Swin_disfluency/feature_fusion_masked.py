...
OUT_DIR = "/data/alc_jihan/feature_fusion_masked_hubert_meta_disfluency"
...
# ====== 4) Feature Fusion (시퀀스 전체 유지 + 마스킹) 및 저장 ======
...
for idx, row in df.iterrows():
    ...
    # 각 프레임에 대해 복제 → (T, 5) if 적용, 아니면 모두 0
    if task in TARGET_TASKS:
        disflu_norm_expanded = np.tile(disflu_norm, (T, 1))
        mask_disfluency = np.ones((T, len(DISFLUENCY_COLS)), dtype=np.float32)
    else:
        disflu_norm_expanded = np.zeros((T, len(DISFLUENCY_COLS)), dtype=np.float32)
        mask_disfluency = np.zeros((T, len(DISFLUENCY_COLS)), dtype=np.float32)
    ...
    # 6) 마스크 생성: HuBERT 및 metadata 부분는 항상 1, disfluency 부분은 위에서 결정한 mask 사용
    mask_hubert = np.ones((T, normalized_hubert.shape[1]), dtype=np.float32)  # (T,1024)
    mask_meta = np.ones((T, meta_norm_expanded.shape[1]), dtype=np.float32)      # (T,2)
    mask_vec = np.concatenate([mask_hubert, mask_meta, mask_disfluency], axis=1)   # (T,1031)
    
    # 7) 결과를 dict에 묶어 저장
    fused_tensor = torch.from_numpy(fused_vec).float()
    mask_tensor = torch.from_numpy(mask_vec).float()
    final_dict = {
        "fused_feature": fused_tensor,  # shape: (T,1031)
        "mask": mask_tensor             # shape: (T,1031)
    }
